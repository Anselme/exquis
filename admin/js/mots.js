...
$('.supprimer_mot').live('click', function (e) {	
	var conteneur = $(this).parent().parent();
	var mots = $(this).html();
	var tab_id = $(this).attr("class").split('-');
	var mot_id = tab_id[1];
	alertify.confirm("Supprimer cette expression : \""+mots+"\"", function (e) {
    	if (e) {
        	// user clicked "ok"
			var dataString = "id="+mot_id;
			if (mot_id !== "") {
				$.ajax({
      				type: "POST",
      				url: "supprimer_mots.php",
      				data: dataString,
      				success: function(data) {
						if (data === "ok") {
							alertify.alert("L\'expression a bien \351t\351 supprim\351 !");
							conteneur.remove();
						}
						else {
							alertify.alert(data);
